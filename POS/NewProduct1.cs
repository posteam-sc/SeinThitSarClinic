...
namespace POS
{
    public partial class NewProduct1 : Form
    {
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            ...
            //    return true;
            //}
            if ((txtBarcode.Focused) && (keyData == (Keys.Enter) || keyData == (Keys.Tab)))
            {
                txtProductCode.Focus();
                return true;
            }
            else if ((txtProductCode.Focused) && (keyData == (Keys.Enter) || keyData == (Keys.Tab)))
            {
                txtName.Focus();
                return true;
            }
            else if ((txtName.Focused) && (keyData == (Keys.Enter) || keyData == (Keys.Tab)))
            {
                txtUnitPrice.Focus();
                return true;
            }
            else if ((txtUnitPrice.Focused) && (keyData == (Keys.Enter) || keyData == (Keys.Tab)))
            {
                txtQty.Focus();
                return true;
            }
             else if ((txtQty.Focused) && (keyData == (Keys.Enter) || keyData == (Keys.Tab)))
            {
                btnSubmit.Focus();
                return true;
            }
